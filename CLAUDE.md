...
| Field | Value |
|-------|-------|
| **Current phase** | Phase 11 (Paper Scaffold + Reproducibility) OPEN. Task 11.1 merged to `main` via PR #39, squash commit `b3b86f9`. |
| **Next task** | **Phase 11 task 11.2 (claim-to-experiment mapping).** Local `main` is synced to `b3b86f9` -- cut `feat/phase11-task11.2-claim-map` FRESH from `main` at session start (do NOT reuse the now-deleted 11.1 branch). **11.2 scope:** (a) rewrite the raw `tabular` in `paper/sections/05_experiments.tex:43-56` into a proper `\begin{table}\label{tab:claim-map}\end{table}` so the `\todo{reference \cref{tab:claim-map} ...}` forward-reference from 11.1 resolves cleanly (the scaffold currently emits an undefined-reference WARNING on every compile, intentional carry-over from 11.1, documented in the 11.1 review); (b) flip every `\todo{\textbackslash includegraphics of fig<N>_*}` placeholder in `paper/sections/06_results.tex` into a real `\includegraphics[width=\columnwidth]{figN_*}` call (Phase 10 figure stems live at `SIGNAL_DRIFT_FIG_STEM` = `fig1_signal_drift`, `HEALTH_COMPARISON_FIG_STEM` = `fig2_health_score_comparison`, `AWT_BOX_FIG_STEM` = `fig3_awt_box_plot`, `ALLOC_EFF_OVER_TIME_FIG_STEM` = `fig4_allocation_efficiency_over_time`, `ROC_CURVE_FIG_STEM` = `fig5_roc_curve`, `ABLATION_BAR_FIG_STEM` = `fig6_ablation_bar_chart`; `\graphicspath` in `main.tex` is already wired); (c) decide the C5 (forecaster overhead) story: either wire a real latency microbenchmark into `src/chronoagent/experiments/experiment_runner.py` (preferred; emits a new row in `aggregate.json` that the 10.8 `make_main_results_table` already has a column for) OR downgrade C5 to a soft claim with a documented caveat. The 11.1 Challenges entry in PLAN.md explicitly defers this call to 11.2, so choose and record. Mirror the 10.9/11.1 PR format (1500 passed baseline + any net-new tests, ruff/ruff format/mypy clean, list files changed in the table). Pre-existing Chroma cross-test-pollution flake in `tests/unit/test_memory_router.py::TestQuarantineDocs::test_quarantine_without_reason` (passes in isolation) may re-trip; same documented pattern from 5.7/8.3/9.x/10.x, not a blocker. |
| **Blocked?** | No |
