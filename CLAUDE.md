...
| **Next task** | 5.3, `allocator/task_allocator.py` (`DecentralizedTaskAllocator`: subscribe to health_updates, maintain snapshot, call negotiation) |
| **Blocked?** | No |
| **Last session** | 2026-04-10. Phase 5 task 5.2 done [x] and pushed to `feat/phase5-task5.2-negotiation` with PR open. New `src/chronoagent/allocator/negotiation.py` is a pure contract-net protocol: `run_contract_net(task_id, task_type, health_snapshot, *, matrix, threshold, missing_health_default) -> NegotiationResult`. Bid = capability * health. Tie-break by strict `>` iterating `AGENT_IDS` in order (first wins, matches `CapabilityMatrix.primary_agent`). Max bid strictly < threshold -> escalate (`assigned_agent=None`, `winning_bid=None`). Module-level `DEFAULT_BID_THRESHOLD=0.25`, `DEFAULT_MISSING_HEALTH=1.0` (optimistic at boot). Frozen dataclasses `Bid` and `NegotiationResult` for audit serialisation. Typed errors `InvalidHealthError` / `InvalidThresholdError` on NaN / out-of-range. Unknown task types bubble up via `CapabilityMatrix.column`. Extra keys in snapshot are ignored. `__init__.py` re-exports the new symbols. Tests in `tests/unit/test_negotiation.py` (54 tests incl. 3 Hypothesis properties): happy-path per task, degraded-specialist redistribution (all 4 directions), tie-breaking, escalation, snapshot edge cases (missing/extra/NaN/out-of-range), threshold boundaries, custom matrix, frozen dataclass assertions, rationale content, one-of-assigned-or-escalated invariant, winner-maximises-score invariant. Total 594 tests pass (540 + 54). Negotiation module 100% covered; allocator module total 93%. All 4 CI checks green locally (ruff lint + format, mypy --strict, pytest). |
