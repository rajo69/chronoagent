...
\paragraph{Problem.}
Vector-memory-backed multi-agent LLM systems, built on frameworks
such as LangChain~\citep{langchain}, LangGraph~\citep{langgraph},
AutoGen~\citep{autogen}, and MetaGPT~\citep{metagpt}, have become the
default architecture for long-horizon tasks (planning, code review,
research assistants). Shared memory is a force multiplier for
throughput but it is also a single high-value attack surface: an
adversary who can write even one malicious document into the shared
store can influence every retrieval-time decision that follows.
Recent work~\citep{minja,agentpoison,amemguard} has shown that such
attacks are both feasible and hard to detect with static input
filters.
...
\begin{enumerate}
  ...
  (\cref{sec:system}).
  \item \textbf{An end-to-end evaluation} against
  MINJA~\citep{minja} and AgentPoison~\citep{agentpoison} with three
  ablations and two reactive baselines, reporting Advance Warning
  Time (\awt{}), efficiency under attack, and ROC curves
