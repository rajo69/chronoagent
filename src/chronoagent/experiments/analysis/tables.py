...
LATENCY_TABLE_STEM: str = "table4_latency"
...
_METRIC_COLUMNS: tuple[tuple[str, str, int], ...] = (
    ...
    ("latency_ms", "Latency (ms/step)", _LATENCY_MS_DECIMALS),
)
...
def make_latency_table(
    ...
    """Generate a focused per-detector latency comparison table.

    This table backs claim C5 (forecaster overhead) in the paper. Each
    row is one experiment configuration; the single data column shows
    mean per-step latency in milliseconds with a 95% CI.
    ...
    Args:
        ...
        output_path: Optional override for the output ``.tex`` file
            location. Defaults to
            ``<results_dir>/tables/<LATENCY_TABLE_STEM>.tex``.
    ...
    key = "latency_ms"
    decimals = _LATENCY_MS_DECIMALS

    rows: list[tuple[str, _MetricCell]] = []
    for name in experiment_names:
    ...
    # Find lowest latency for bolding (lower is better for latency).
    best_mean = float("nan")
    for _, cell in rows:
        if math.isnan(cell.mean):
            continue
        if math.isnan(best_mean) or cell.mean < best_mean:
            best_mean = cell.mean

    lines: list[str] = []
    lines.append("% Auto-generated by chronoagent.experiments.analysis.tables")
    lines.append("% Phase 11 task 11.4 - latency comparison table")
    lines.append("\\begin{tabular}{lc}")
    lines.append("\\hline")
    lines.append("Configuration & Latency (ms/step) \\\\")
    lines.append("\\hline")
    for name, cell in rows:
        rendered = _format_mean_ci(cell.mean, cell.ci_low, cell.ci_high, decimals=decimals)
        if not math.isnan(cell.mean) and not math.isnan(best_mean) and cell.mean == best_mean:
            rendered = f"\\textbf{{{rendered}}}"
        lines.append(f"{_display_name(name)} & {rendered} \\\\")
    lines.append("\\hline")
    lines.append("\\end{tabular}")

    target = _resolve_table_path(results_dir, output_path, LATENCY_TABLE_STEM)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return target
...
def generate_all_tables(
    ...
    *,
    ...
    Returns:
        ...
        main results, ablation, latency, then signal validation if
        supplied.
    """
    paths: list[Path] = []
    ...
    paths.append(make_latency_table(results_dir, main_experiment_names))
