...
**Currently:** Phase 11 task 11.3 populated `paper/bibliography.bib` with 16 real entries and wired inline `\citep` / `\citet` calls into every section of the scaffold. The bib is grouped by the section that introduces each work: attacks (MINJA, AgentPoison, A-MemGuard), allocation (Smith's contract-net, Oliehoek & Amato Dec-POMDP, AutoGen, MetaGPT, CrewAI), change-point and forecasting (Adams & MacKay 2007 BOCPD, Chronos), runtime monitoring (NeMo Guardrails, Llama Guard, Constitutional AI), and the framework stack (LangChain, LangGraph, Chroma). Every `\todo{cite ...}` in `02_related_work.tex` is resolved, and the numeric `natbib` mode in `main.tex` is now backed by real references instead of a one-entry `chronoagent-stub` placeholder. The "Sentinel-style baseline" in `05_experiments.tex` is cited to LlamaGuard + NeMo Guardrails rather than a fabricated entry; the scoping note is logged in the 11.3 Challenges block for a later writing pass to revisit. A Python bib-audit script confirmed zero missing cite keys and zero unused entries before commit. Tasks 11.4 (Makefile `reproduce*` targets + latency harness) and 11.5 (Docker pin + seed freeze) remain. 1500 tests pass with 95.98% line coverage; no source files changed in 11.3, so the test count carries forward unchanged.
