...
**Currently:** Phase 11 task 11.1 landed the paper scaffold. The `paper/` directory now holds `main.tex` (article class, natbib + cleveref, `\graphicspath` wired to every per-experiment and multi-experiment figures directory under `../results/`), nine section files under `paper/sections/` (`00_abstract` through `08_conclusion`), and a stub `bibliography.bib` that keeps `pdflatex && bibtex && pdflatex && pdflatex` green until task 11.3 populates real entries. A `\inputiffound` helper guards every `\input{../results/tables/*.tex}` so the scaffold compiles even when the Phase 10 results are stale or absent, surfacing a visible TODO in the PDF instead of a hard error. Tasks 11.2 (claim-to-experiment wiring), 11.3 (bibliography), 11.4 (Makefile `reproduce*` targets), and 11.5 (Docker pin + seed freeze) remain. 1500 tests pass with 95.98% line coverage; no source files changed in 11.1, so the test count carries forward unchanged.
